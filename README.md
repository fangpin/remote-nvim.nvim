...
```lua
 {
  ...
  remote = {
    ...
    reconnect = {
      enabled = false, -- Automatically relaunch the remote Neovim session if the SSH job exits unexpectedly.
  ...
  -- starts a terminal remote UI with `nvim --remote-ui`.
...
`remote.reconnect` can automatically relaunch the current workspace when the remote Neovim SSH job exits unexpectedly,
for example after a transient network drop. It does not trigger when you explicitly stop the session with
`:RemoteStop`.
