...
describe("Shell path escaping", function()
  it("keeps bare tilde paths expandable", function()
    assert.equals("~", utils.shell_escape_path("~"))
    assert.equals("~root", utils.shell_escape_path("~root"))
  end)

  it("keeps tilde prefixes expandable while escaping the remainder", function()
    assert.equals("~/'foo bar'", utils.shell_escape_path("~/foo bar"))
    assert.equals("~root/'project dir/file name'", utils.shell_escape_path("~root/project dir/file name"))
  end)

  it("shell-escapes non-tilde paths normally", function()
    assert.equals("'/tmp/foo bar'", utils.shell_escape_path("/tmp/foo bar"))
  end)
end)
